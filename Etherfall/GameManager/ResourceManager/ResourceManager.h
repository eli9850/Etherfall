...
namespace Etherfall {
	...
	public:
		...
		std::filesystem::path get_npcs_details_dir();
		const nlohmann::json& get_npc_details_by_id(uint32_t npc_id);
	...
	private:
		...
		std::filesystem::path get_npc_path(uint32_t npc_id);
		...
		const sf::Texture& add_npc(uint32_t npc_id);
	private:
